...
class Square:
    ...
    def copy(self):
        return Square(file=self.file, rank=self.rank)

    @classmethod
    def from_lan(cls, lan: str):
        f, r = lan
        return cls(file=ord(f)-ord('a')+2, rank=int(r)+1)


class Move:
    def __init__(self, start: Square, end: Square, promote = None):
        self.start = start
        self.end = end
    ...
    def copy(self):
        return Move(start=self.start.copy(), end=self.end.copy())

    @classmethod
    def from_lan(cls, lan: str, whites_turn: bool):
        if len(lan)==5:
            promote = dict(
                q=QUEEN, b=BISHOP, k=KNIGHT, r=ROOK
            )[lan[4]] | (WHITE if whites_turn else BLACK)
        else:
            promote = None
        return cls(
            start=Square.from_lan(lan[0:2]),
            end=Square.from_lan(lan[2:4]),
            promote=promote,
        )
...
class State:
    ...
    def __repr__(self):
        fen = ""
        for rank in range(RANK_8, RANK_1-1, -1):
            spaces = 0
        ...
        fen += f' {self.halfmovecounter} {self.fullmovecounter}'
        return fen

    def _to_san(self, lan: str):
        move = Move.from_lan(lan, self.whites_turn)
        piece = self[move.start]
        if piece & KING:
            if lan in ["e1h1", "e8h8"]:
                return "O-O"
            elif lan in ["e1a1", "e8a8"]:
                return "O-O-O"
        out = piece_char[piece] if piece & PAWN == 0 else ''
        if self[move.end] != EMPTY:
            if piece & PAWN:
        ...
        if next_state.is_mate():
            out += '#'
        elif next_state.is_mine_check():
            out += '+'
        return out

    def to_san(self, lan: str) -> str:
        move = Move.from_lan(lan, self.whites_turn)
        piece = self[move.start]
        target = self[move.end]
        ...
        if piece & PAWN:
            if move.start.file != move.end.file:
                san_move = f"{str(move.start)[0]}x{str(move.end)}"
            else:
                san_move = str(move.end)
        else:
            # Determine if disambiguation is needed
            ambiguous = []
            for y in range(8):
                for x in range(8):
                    other_square = Square(file=x+PAD, rank=y+PAD)
                    if move.start == other_square:
                        continue
                    other_move = Move(other_square, move.end)
                    if self[other_square] == piece:
                        # Check if another piece of the same type can move to the same square
                        if self.check_move(other_move):
                            ambiguous.append(other_square)
            san_move = piece_char[piece]
            if ambiguous:
                if move.start.file not in [other.file for other in ambiguous]:
                    san_move += str(move.start)[0]
                elif move.start.rank not in [other.rank for other in ambiguous]:
                    san_move += str(move.start)[1]
                else:
                    san_move += str(move.start)
            if target != EMPTY:
                san_move += 'x'
            san_move += str(move.end)
    ...
    def copy(self):
        ...
        new_state.halfmovecounter = self.halfmovecounter
        new_state.fullmovecounter = self.fullmovecounter
    ...
    def check_pawn_move(self, move: Move):
        ...
        if (delta == Delta(0, 2 * dir)) and (move.start.rank == (RANK_2 if self.whites_turn else RANK_7)):
            ...
            if move.end.rank == (RANK_8 if self.whites_turn else RANK_1):
                return self[move.end] == EMPTY and move.promote is not None
            else:
                return self[move.end] == EMPTY
        elif delta == Delta(1, dir) or delta == Delta(-1, dir):
            if move.end.rank == (RANK_8 if self.whites_turn else RANK_1):
                return self[move.end] & self.oppo and move.promote is not None
            else:
    ...
    def move(self, move: Move):
        ...
        if self[move.end] is not EMPTY or move.end == self.enpassant:
            self.halfmovecounter = 0
        ...
        for delta in [Delta(0, 2 * dir), Delta(0, dir), Delta(-1, dir), Delta(1, dir)]:
            ...
            if move.end.rank == (RANK_8 if self.whites_turn else RANK_1):
                for promote in [QUEEN, ROOK, BISHOP, KNIGHT]:
                    promoting_move = move.copy()
                    promoting_move.promote = self.mine | promote
                    if self.check_move(promoting_move):
                        moves.append(promoting_move)
    ...
    def move_must_promote(self, move):
        return self[move.start] & PAWN and move.end.rank == (RANK_8 if self.whites_turn else RANK_1)
