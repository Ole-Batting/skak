...
from skak.game_state import WHITE_PIECE, BLACK_PIECE, OTHER_PIECE, Square, State
...
class Board:
    ...
    def blit_moves(self, state: State, square: Square):
        for move in state.generate_moves(square):
            if state.is_opponent(move.end):
                self.place(self.capture_overlay, move.end.file.value, move.end.rank.value)
            else:
                self.place(self.move_overlay, move.end.file.value, move.end.rank.value)
